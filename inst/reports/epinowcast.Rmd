...
In order to inspect how our model is doing we might also want to plot observations against posterior predictions summarised by the delay from onset to report.

```{r plot-posterior-predictions}
#| fig-cap: The solid line with points is the observed data. The dashed line is
#| the median of summarised predictions and the ribbons are the 60% and 80%
#| credible intervals. Note these are estimated by adding individual prediction
#| quantiles and so likely over estimate uncertainty.
...
In real-world data we might expect to see time-varying delays in reporting, overdispersed reporting, and the likelihood of reporting varying by report date (i.e due to the day fo the week ec.). All of these complexities can be modelled using `epinowcast 0.1.0`.
